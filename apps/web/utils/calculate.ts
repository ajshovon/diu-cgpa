import { SingleResult } from "@diu-cgpa/types";

const getStudentInfo = async (id: string, apiBase: string) => {
  try {
    const response = await fetch(`${apiBase}/api/result/full?id=${id}`);
...
export const calculateCgpa = async (id: string, improvement: boolean, checkProject: boolean, projectCredit: number, projectResult: number, apiBase: string) => {
  const { inc, studentInfoObj, semesterResults } = await getStudentInfo(id, apiBase);
  let credits = 0;
  let weighted = 0;
  const resultsList = [];
