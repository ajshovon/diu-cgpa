...
import { StudentInfo } from "@diu-cgpa/types";
...
app.get(
  ...
  async (c) => {
    ...
    if (responseInfo.status !== 200) {
      throw new HTTPException(responseInfo.status as ContentfulStatusCode);
    }
    const dataInfo = (await responseInfo.json()) as StudentInfo;
    let startSemester = parseInt(dataInfo["semesterId"] as string);
